    ...
    <div v-if="step == 2" style="padding-bottom: 90px">
      ...
      <div class="box">
        <div style="margin: 48px 0 0 108px">
          <div class="item">
            <p style="font-size: 18px">{{isETHToBSC ? "Ethereum" : "Binance Smart Chain"}}</p>
<!--            <el-input-->
<!--              style="width: 420px"-->
<!--              v-model="address"-->
<!--            ></el-input>-->
            ...
            >
              <template v-if="isETHToBSC">
                <el-option
                  v-for="item in ethList"
                  :key="item.name"
                  :label="item.name"
                  :value="item.name"
                >
          ...
          <div class="item">
            <p style="font-size: 18px">{{isETHToBSC ? "Binance Smart Chain" : "Ethereum"}}</p>
<!--            <el-input-->
<!--              style="width: 420px"-->
<!--              v-model="balance"-->
<!--            ></el-input>-->
          ...
          <div class="item">
            <p style="font-size: 14px">{{isETHToBSC ? "ETH" : "BNB"}} {{ $t("balance") }}</p>
            <el-input
              style="width: 420px"
              readonly
              v-model="balance"
...
export default {
  ...
  methods: {
    changeDirection() {
      console.log("is ETH to BSC", this.isINTToETH, this.fromCoin, this.toCoin)
      this.isETHToBSC = !this.isETHToBSC;
      [this.fromCoin, this.toCoin] = [this.toCoin, this.fromCoin];
      console.log("is ETH to BSC", this.isETHToBSC, this.fromCoin, this.toCoin)
      let list = this.isETHToBSC ? this.ethList : this.bscList;
    ...
    changeFromOptions(param) {
      ...
      if (this.isETHToBSC) {
        this.ethList.forEach((value, index) => {
          if(value.name === param) {
        ...
        this.bscList.forEach((value, index) => {
          if(value.name === param) {
            console.log("value", value);
            this.toCoin = this.ethList[index].name;
            this.fromContractAddr = value.address;
          }
        })
      }
    ...
    changeToOptions(param) {
      ...
      if (this.isETHToBSC) {
        this.bscList.forEach((value, index) => {
          if(value.name === param) {
            console.log("value", value);
            this.fromCoin = this.ethList[index].name;
            this.fromContractAddr = this.ethList[index].address;
          }
        })
      } else {
        this.ethList.forEach((value, index) => {
    ...
    unlock(account) {
      ...
      this.step = 2;
      this.fromContractAddr = this.ethList[0].address;
      this.getBalance(this.address);
      this.getTokenBalance(this.address, this.fromContractAddr)
    },
    ...
    getBalance(address) {
      if (this.isETHToBSC) {
        erpc.getBalance(address).then((res) => {
          console.log("erpc get balance", res);
    ...
    getTokenBalance(address, contractAddress) {
      ...
      if (this.isETHToBSC) {
        erpc.getTokenBalance({address, contractAddress}).then(async (res) => {
          this.tokenDecimals = await erpc.getTokenDecimals(contractAddress);
    ...
    sendTx() {
      ...
      let transferAmount = this.tokenBalance;
      ...
      // if (this.isINTToETH && this.fromContractAddr === "") {
      //   // transfer INT
